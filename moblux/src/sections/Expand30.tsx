...
const GALLERY = [
  {
    src: IMAGES.expand30Exterior,
    alt: "30 lábas kinyitható ház tetővel és fa hatású burkolattal",
    wide: true,
  },
  {
    src: IMAGES.expand30Interior1,
    alt: "30 lábas kinyitható ház bézs külső változata",
  },
  {
    src: IMAGES.expand30Interior3,
    alt: "30 lábas kinyitható ház fa hatású külső változata",
  },
    ...
    <section id="expandable30" className="pad bg-[#ebe8e1]">
      ...
      <Reveal className="grid min-h-[650px] bg-ink-panel text-white max-lg:grid-cols-1 lg:grid-cols-[1.15fr_0.85fr]">
        ...
        <div className="self-center px-[52px] py-[60px] max-sm:px-6 max-sm:py-[42px]">
          ...
          <p className="text-[#b9bbb5]">
            A 30 lábas központi egység szállításkor mindössze 2,2 méter széles,
            kinyitva pedig 6,3 méteres épületszélességet és közel 57 m²-es
            alapterületet biztosít. Lakóháznak, nyaralónak, vendégháznak vagy
            befektetési célra is személyre szabható.
          </p>
          <div className="my-[30px] grid grid-cols-2 border-t border-[#444] max-sm:grid-cols-1">
            {NUMBERS.map(([value, label]) => (
              <div
                key={label}
                className="border-b border-[#333] pt-[15px] pr-3 pb-2"
              >
                <b className="block text-gold-light">{value}</b>
                <span className="block text-[9px] uppercase text-[#888]">
                  {label}
                </span>
              </div>
            ))}
      ...
      <Reveal className="mt-px grid min-h-[560px] bg-white max-lg:grid-cols-1 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="self-center p-[55px] max-sm:px-6 max-sm:py-[42px]">
          ...
          <p className="note">
            A bemutatott alaprajz egy választható elrendezés. A helyiségek száma
            és kiosztása az egyedi megrendelés szerint módosítható.
          </p>
        </div>
      </Reveal>
      ...
      <p className="note">
        A képek és műszaki adatok a gyártói referenciakivitelt mutatják. A
        MOBLUX-változat alaprajza, szigetelése, burkolatai, nyílászárói és
        felszereltsége személyre szabható.
      </p>
    </section>
  );
}
