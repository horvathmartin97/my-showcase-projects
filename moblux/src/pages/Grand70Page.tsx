...
const GALLERY = [
  {
    src: IMAGES.grand70Built,
    alt: "Elkészült MOBLUX Grand 70 kétszintes kivitel",
    span: "row-span-2",
  },
  {
  ...
  {
    src: IMAGES.twoStory04,
    alt: "Grand 70 hátsó homlokzat",
    span: "col-span-2",
  },
  { src: IMAGES.floorplan, alt: "alaprajz" },
];
  ...
  return (
    <>
      <section className="relative h-[100svh] min-h-[730px] overflow-hidden text-white">
        <Img
          src={IMAGES.twoStory02}
          alt="MOBLUX Grand 70 kétszintes otthon"
          loading="eager"
        ...
        <Reveal className="absolute bottom-[25%] left-[max(24px,calc((100vw_-_var(--max))/2))] z-2 w-[min(850px,calc(100%_-_48px))] max-sm:bottom-[54px] max-sm:left-5 max-sm:w-[calc(100%_-_40px)]">
          ...
          <p className="mt-6 max-w-[700px] text-[clamp(17px,1.5vw,21px)] text-[#e4e3df]">
            Közel 70 m²-nyi világos, karakteres élettér nagy üvegfelületekkel és
            személyre szabható belső kialakítással.
          </p>
      ...
      <Reveal as="section" id="details" className="pad bg-white">
        ...
        <div className="grid items-end gap-[70px] max-lg:grid-cols-1 max-lg:gap-[22px] lg:grid-cols-[1.35fr_0.65fr]">
          ...
          <div className="text-[17px] text-muted">
            <p>
              A MOBLUX Grand 70 azoknak készült, akik kis telepítési helyigény
              mellett szeretnének tágas, látványos otthont, vendégházat vagy
              befektetési célú szálláshelyet.
            </p>
            <p>
              A szendvicspanel külső burkolat, a nagy dupla üvegezésű
              nyílászárók és a két terasz kortárs, könnyen felismerhető
              karaktert adnak.
            </p>
          </div>
        </div>
      ...
      <section className="pad bg-ink-panel text-white">
        ...
        >
          ...
          <p className="text-[17px] text-[#bbb]">
            A belső elrendezés, a burkolatok és a felszereltség a projekt
            céljához igazítható.
          </p>
        </Reveal>
        ...
        <dl className="grid border-t border-[#41423e] max-sm:grid-cols-2 sm:grid-cols-4">
          {NUMBERS.map(([value, label]) => (
            <div
              key={label}
              className="border-r border-[#41423e] p-8 max-sm:p-[22px]"
            >
              <dt className="text-[30px] text-gold-light">{value}</dt>
              <dd className="m-0 text-[10px] tracking-[0.12em] text-[#999] uppercase">
                {label}
              </dd>
            </div>
          ))}
      ...
      <section id="model-gallery" className="pad bg-white">
        ...
        <p className="note">
          A hosszú fedett oldalteraszos környezetkép illusztráció; a végleges
          kialakítás és felszereltség minden projektben személyre szabható.
        </p>
      </section>
      ...
      <section className="bg-[#cdbf9f] px-[5vw] py-[115px] text-center max-sm:py-20">
        ...
        <p className="mx-auto mb-[30px] max-w-[640px]">
          Mondja el, milyen célra használná, és összeállítjuk az egyedi műszaki
          tartalmat.
        </p>
        <Link to="/ajanlatkeres" className="btn btn-gold">
